...
# by Using options()! or shinyOptions()

# How does each type of widget differ in its code and how it references the dataframe?
#They all have inputs, and labels, and then options for the values that are used and useful to the widget.

#2. How is the mainPanel component of the UI structured? 
# Its just a scatterplot based on the inputs and a table based on a reactive element referencing the scatterplot
# How does the output appear based on this code?
# You get the empty table and the scatterplot showing the data according to your selection.

#3. Explore the reactive formatting within the server.
# Which variables need to have reactive formatting? 
#The inputs to the scatterplot!
# How does this relate to selecting rows vs. columns from the original data frame?
#it's basically the same thing, we are taking rows based off of certain column selection options in the sidebar.
...
#such that they can be properly accessed in a dynamic environment!
