...
ggplot(BRA_complete)+
  ...
  geom_smooth(aes(x=Date, y=Rainfall))
...
BRA_ts_rain <- ts(BRA_complete$Rainfall,start=c(1901,01),frequency=12)
BRA_ts_temp <- ts(BRA_complete$Temperature_C,start=c(1901,01),frequency=12)
...
BRA_stl_rain <- stl(BRA_ts_rain,s.window = 13)
BRA_stl_temp <- stl(BRA_ts_temp,s.window=13)

plot(stl(BRA_ts_rain,s.window = 13))
plot(stl(BRA_ts_temp,s.window=13))
...
BRA_STL_DF_Rain <- as.data.frame(BRA_stl_rain$time.series)
BRA_STL_DF_Temp <- as.data.frame(BRA_stl_temp$time.series)
#BRA_STL <- BRA_complete%>%mutate(,)
BRA_
